    ...
    public void read(Operation op){
        ...
        if(upSites.isEmpty()){
            ...
            if(isLockedCount(upSites, op) == 0){ //all unlocked
                ...
                //parsing
                LOGGER.fine("WRITE LOCK BY" +variable.getLockedByTransaction().get(0)+ ", Adding[" +op+ "] to waitQueue");
                waitQueue.add(op);
                //Add to waitforgraph
                //deadlock detection
            }
    ...
    public void write(Operation op) {
        ...
        if(upSites.isEmpty()){
            ...
            if(isLockedCount(upSites, op) == 0){ //all unlocked
                ...
                if(variable.getLockType() == LockType.WRITELOCK && op.getTransaction() == variable.getLockedByTransaction().get(0)) {
                    ...
                    if(variable.getLockedByTransaction().size() ==1 && variable.getLockedByTransaction().get(0).getTransactionNumber() == op.getTransaction().getTransactionNumber()){
                        ...
                        //parsing
                        LOGGER.fine("READ LOCK by multiple transactions/other transaction, Adding [" +op+ "] to waitQueue");
                        waitQueue.add(op);
                        //Add to waitforgraph
                        //deadlock detection
                    }
